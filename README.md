...
It checks loaded Workshop items, logs update details, warns players, disconnects clients before saving, waits for the save to finish, and quits the server process so an external supervisor can restart it. It can also run scheduled restarts on a daily clock time or a fixed hour interval.

BSU quits the server. **It does not relaunch it.** Something outside the game has to bring it back up, which is what the included wrapper scripts are for.

---

## Quick start

1. Subscribe to the mod and add `BSU` to your server mod list.
2. Set up a restart wrapper (below) so the server comes back after BSU quits.
3. Configure restart behavior in Sandbox Options.
4. Optionally test with `QuitServer=false` first. That runs the entire lifecycle without actually quitting.
...
The wrappers are convenience, not a requirement. Anything that restarts a dead process works:

- systemd service with `Restart=always`
- Docker restart policy
- AMP/Pterodactyl/other server panel restart behavior
- Windows service wrapper
- your own batch/shell loop

Relaunching the same dedicated server process from inside the already-terminating process is not a reliable supported path, which is why BSU leaves this to an external supervisor.
...
BSU can trigger the same restart lifecycle without waiting for a Workshop update or admin command. `ScheduledRestartMode` has three sandbox choices:
